...
class AccountService:
    ...
    async def get_active_accounts(db: AsyncSession, user_id: UUID):
        """
        Retrieve all accounts belonging to a user that are not soft-deleted.
        Returns a list of dictionaries (or SQLModel objects) – adapt as needed.
        """
        ...
        return [
            ...
            for acc in accounts if acc.deleted_at is None
        ]

    @staticmethod
    async def soft_delete_account(db: AsyncSession, account_id: UUID) -> bool:
        """
        Mark an account as deleted (soft delete) if it exists and is not
        already deleted.
        """
        stmt = select(Account).where(
